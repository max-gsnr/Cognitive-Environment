...
function verdict(childName: string, metrics: SessionMetrics): string {
  ...
  if (metrics.challenge_fit >= 0.6) {
    return `${name} is in the target zone on ${pct(metrics.challenge_fit)} of recent questions and recovering from mistakes in ${metrics.mean_recovery_questions ?? "—"} questions. Leave it alone.`;
  }
