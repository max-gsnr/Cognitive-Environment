...
(function () {
  ...
  // Pacing: a correct answer moves on quickly so mastered material keeps its
  // rhythm; a gentle correction stays on screen long enough to actually read.
  // Both are fixed pauses, never a visible clock.
  const ADVANCE_AFTER_CORRECT_MS = 650;
  const ADVANCE_AFTER_GENTLE_MS = 1600;
  // Instant per-action reward, varied so the praise itself stays interesting.
  const PRAISE = [
    "Docked!",
    "Smooth landing!",
    "Pods joined!",
  ...
  async function submitAnswer(value) {
    ...
    if (result.is_correct) {
      blip(660);
      showFeedback(PRAISE[praiseIndex % PRAISE.length], false);
      praiseIndex += 1;
    } else {
  ...
  el.form.addEventListener("submit", async (event) => {
    ...
    el.submit.disabled = true;
    try {
      const result = await submitAnswer(Number(raw));
      const pause = result.is_correct
        ? ADVANCE_AFTER_CORRECT_MS
        : ADVANCE_AFTER_GENTLE_MS;
